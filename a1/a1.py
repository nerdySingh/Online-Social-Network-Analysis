...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    parent =[]
    child =[]
    for i in range(len(node2parents)):
          if node2parents[i][0] == root:
                root1.append(node2parents[i][0])
          else:
                if root == node2parents[i][1]:
                      root1.remove(root)
                      root1.apppend(node2parents[1][0])
                      parent.append(node2parents[1][1])
                      child.append(node2parents[0][1])
                else:
                      parent.append(node2parents[1][0])
                      child.append(node2parents[0][1])

    #print("node2parents:",node2parents,"root1:",root1,"parent:",parent,"child:",child)
    result =[]
    ...
    for i in range(len(root1)):
          fract = fract+1
          temp = tuple(((root1[i],parent[i]),fract))
          result.append(temp)
...
def partition_girvan_newman(graph, max_depth):
    ...
    if graph.order() ==1:
      return [graph.nodes]
    def find_best_edge(G,max_depth):
...
def get_subgraph(graph, min_degree):
    ...
    new_list =[]
    for i in range(len(result)):
      if graph.degree(result[i]) == min_degree or graph.degree(result[i]) > min_degree:
        new_list.append(result[i])
    return graph.subgraph(new_list)
    #print(result)
...
def score_max_depths(graph, max_depths):
    ...
    for i in max_depths:
          data = partition_girvan_newman(graph,i)
          print(data)
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    for i in range(len(neighbors)):
      temp = tuple((test_node,neighbors[i]))
      nn.append(temp)
    graph_cp.remove_edges_from(nn[:n])
    #print(neighbors)
    return graph_cp
    ###TODO
    pass
...
def jaccard(graph, node, k):
    ...
    for i in range(len(scores)):
          temp=tuple(((node,scores[i][0]),scores[i][1]))
          ml.append(temp)
    ne_ml=[] 
    flag = False         
...
def path_score(graph, root, k, beta):
    ...
    for i in range(len(nodes)):
          blah = bfs_shortest_path(graph,root,nodes[i])
          result.append(blah)
          ln.append(list(nx.all_shortest_paths(graph,source=root,target=nodes[i])))
          ...
          leng = len(ln)
          leng +=1 
          temp = math.pow(beta,i) * leng
          temp1 = tuple(((root,nodes[i]),temp))
          result.append(temp1)
          '''
    ...
    for i in range(len(result1)):
          for j in range(len(edges)):
                if edges[j]==result1[i][0]:
                      flag =True
                      if flag ==True:
                            ne_ml.append(result[i])
                      else:
                            flag = False
